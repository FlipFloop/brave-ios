...
class FeedCardCell<View: UIView>: UICollectionViewCell {
    var view = View()
...
class SmallHeadlineFeedView: UIView {
    
    let leftFeed = HeadlineCardView().then {
        $0.feedView.titleLabel.numberOfLines = 4
    }
    
    let rightFeed = HeadlineCardView().then {
        $0.feedView.titleLabel.numberOfLines = 4
    }
    ...
    override init(frame: CGRect) {
        ...
        let stackView = UIStackView(arrangedSubviews: [leftFeed, rightFeed]).then {
            $0.spacing = 16
